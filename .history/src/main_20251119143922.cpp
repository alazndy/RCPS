...
const float MAX_FORWARD_RANGE_M = 17.0;
...
void handleDetection(const twai_message_t& msg) {
  ...
  float doc_x_m = msg.data[2] * 0.25;             // İleri (X ekseni)
  float doc_y_m = ((int)msg.data[3] - 128) * 0.25;// Yanal (Y ekseni)
  
  RADAR_PRINTF("  [Veri] Mesafe: %.2fm, Açı: %d, X: %.2fm, Y: %.2fm\n", polarRadius_m, polarAngle_deg, doc_x_m, doc_y_m);

  // =================================================================================
  // DÜZELTME: Nextion Arkaplanları ile Birebir Eşleşme
  // =================================================================================
  // Senin verdiğin bilgilere göre:
  // > 5m   -> Genişlik 10m (Yarıçap 5.0m)
  // 3-5m   -> Genişlik 8m  (Yarıçap 4.0m)
  // 1.5-3m -> Genişlik 6m  (Yarıçap 3.0m)
  // < 1.5m -> Genişlik 4m  (Yarıçap 2.0m)
  // =================================================================================

  float currentLateralRangeRadius_m; // Hesaplamada kullanılacak Yarıçap (Genişlik / 2)
  int backgroundPicId, targetColor;

  if (autoZoom_enabled) {
      if (polarRadius_m > 5.0) { 
          backgroundPicId = PIC_ID_SAFE; 
          currentLateralRangeRadius_m = 5.0; // Toplam Genişlik 10m
          targetColor = COLOR_GREEN; 
      } 
      else if (polarRadius_m > 3.0) { 
          backgroundPicId = PIC_ID_WARNING; 
          currentLateralRangeRadius_m = 4.0; // Toplam Genişlik 8m
          targetColor = COLOR_YELLOW; 
      } 
      else if (polarRadius_m > 1.5) { 
          backgroundPicId = PIC_ID_DANGER; 
          currentLateralRangeRadius_m = 3.0; // Toplam Genişlik 6m
          targetColor = COLOR_ORANGE; 
      } 
      else { 
          backgroundPicId = PIC_ID_ALARM; 
          currentLateralRangeRadius_m = 2.0; // Toplam Genişlik 4m
          targetColor = COLOR_RED; 
      }
  } else {
      // AutoZoom kapalıysa varsayılan en geniş açıyı (10m) kullan
      currentLateralRangeRadius_m = 5.0; 
  ...
  // =================================================================================
  // PİKSEL HESAPLAMA DÜZELTMESİ
  // =================================================================================
  
  // 1. X (Yanal) Konum Hesabı
  // Mantık: doc_y_m (Yanal Uzaklık) / Yarıçap
  // Örn: Cisim 2m sağda, Ekran Genişliği 8m (Yarıçap 4m) -> 2/4 = 0.5 (Ekranın yarısı kadar sağda)
  float scale_x = doc_y_m / currentLateralRangeRadius_m;
  
  int targetX_px = (int)(((float)SCREEN_WIDTH_PX / 2.0) + (scale_x * ((float)SCREEN_WIDTH_PX / 2.0)) + 0.5);
  
  // 2. Y (İleri) Konum Hesabı
  // ÖNEMLİ AYAR: Nextion ızgarasında en üst çizgi kaç metreyi gösteriyor?
  // Eğer ızgaranız 0'dan 10 metreye kadarsa MAX_FORWARD_RANGE_M değerini 10.0 yapmalısınız!
  // Şu anki kodda 7.0 metreye ayarlı.
  int targetY_px = (int)(((MAX_FORWARD_RANGE_M - doc_x_m) / MAX_FORWARD_RANGE_M) * SCREEN_HEIGHT_PX);
  
  // Sınırlandırma (Ekranda kalmasını sağla)
  ...
  updateVehicleDisplay(currentLateralRangeRadius_m); // Genişlik değil Yarıçap gönderiyoruz, fonksiyon içinde 2 ile çarpılıyor.

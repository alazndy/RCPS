...
void handleNextionInput() {
  if (SerialNextion.available()) {
    ...
    // --- KRİTİK DÜZELTME: Çöp Karakter Temizliği ---
    // Tamponun başındaki harf olmayan (A-Z dışındaki) karakterleri atla.
    // Örn: "ÿLOGIN:" gelirse, pointer'ı "L" harfine kaydırır.
    char* cmdStart = rxBuffer;
    while (*cmdStart && !isalpha(*cmdStart)) {
        cmdStart++; 
    }
    
    // Eğer geçerli bir komut bulamadıysa çık
    if (strlen(cmdStart) == 0) return;

    NEXTION_PRINTF("\n[NEXTION] Ham: \"%s\" -> Islenen: \"%s\"\n", rxBuffer, cmdStart);

    // --- KOMUTLARI İŞLE (Artık 'rxBuffer' yerine 'cmdStart' kullanıyoruz) ---

    // LOGIN:1234
    if (strncmp(cmdStart, "LOGIN:", 6) == 0) {
      if (strcmp(cmdStart + 6, password) == 0) {
        NEXTION_PRINTF("[LOGIN] Basarili.\n");
        sendSettingsToNextion();
        sendCommand("page pageSet1");
      } else {
        NEXTION_PRINTF("[LOGIN] Hatali Sifre Denemesi.\n");
    ...
    // SETPASS:1234
    else if (strncmp(cmdStart, "SETPASS:", 8) == 0) {
      char* newPass = cmdStart + 8;
      // Sondaki boşluk veya görünmez karakterleri temizle
      char* end = newPass + strlen(newPass) - 1;
      while(end > newPass && !isdigit(*end) && !isalpha(*end)) {
          *end = '\0';
          end--;
      }
    ...
    // SAVE1:50,20
    else if (strncmp(cmdStart, "SAVE1:", 6) == 0) {
      char* ptr = cmdStart + 6;
      ...
      if (val1 && val2) {
        ...
        saveSettingsToEEPROM();
        NEXTION_PRINTF("[SAVE1] Ok.\n");
      }
    }
    
    // SAVE2:5,20,100
    else if (strncmp(cmdStart, "SAVE2:", 6) == 0) {
      char* ptr = cmdStart + 6;
      ...
      if (vMargin && vWidth && vMax) {
        ...
        saveSettingsToEEPROM();
        NEXTION_PRINTF("[SAVE2] Ok.\n");
      }
    }
    
    // SAVE3:1,0 (Ses Sorunu Burada Çözülüyor)
    else if (strncmp(cmdStart, "SAVE3:", 6) == 0) {
      char* ptr = cmdStart + 6;
      ...
      if (vZoom && vAudio) {
        ...
        // Ses kapatıldıysa DONANIMSAL olarak hemen sustur
        if (!newAudioState) {
            digitalWrite(BUZZER_PIN, LOW);
            buzzerIsOn = false;
    ...
    // RESETALL
    else if (strncmp(cmdStart, "RESETALL", 8) == 0) {
      resetToDefaults();
    }
  }
}

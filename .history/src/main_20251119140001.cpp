...
void handleDetection(const twai_message_t& msg) {
  ...
  // 1. Ham Veriyi Fiziksel Büyüklüklere Çevirme
  // Not: 0.25 katsayısı radarın çözünürlük (resolution) değeridir.
  float polarRadius_m = msg.data[0] * 0.25;       // Radyal Mesafe
  int   polarAngle_deg = (int)msg.data[1] - 128;  // Açı (- değerler sol, + değerler sağ)
  float doc_x_m = msg.data[2] * 0.25;             // İleri Mesafe (X ekseni)
  float doc_y_m = ((int)msg.data[3] - 128) * 0.25;// Yanal Mesafe (Y ekseni)
  
  RADAR_PRINTF("  [Veri] Mesafe: %.2fm, Açı: %d, X: %.2fm, Y: %.2fm\n", polarRadius_m, polarAngle_deg, doc_x_m, doc_y_m);

  // 2. Bölge ve Görsel Durum Kararı
  float currentLateralRange_m;
  int backgroundPicId, targetColor;

  if (autoZoom_enabled) {
      // AutoZoom: Mesafe azaldıkça ekranın gösterdiği yanal genişliği daraltarak odağı artırır.
      if (polarRadius_m > warningZone_m) { 
          backgroundPicId = PIC_ID_SAFE; 
          currentLateralRange_m = lateralRange_L1; 
          ...
          currentLateralRange_m = lateralRange_L4; 
  ...
  // 3. Sesli Uyarı (Buzzer) Mantığı
  // Sadece cisim aracın genişliği + güvenlik payı içindeyse öter (Kaldırım taşlarına ötmesin diye)
  if (audioAlarm_enabled && (polarRadius_m < warningZone_m) && (abs(doc_y_m) < (vehicleRealWidth_m / 2.0 + sideMargin_m))) {
    ...
    if (polarRadius_m <= SOLID_TONE_DISTANCE_M) {
      currentBeepInterval = 0; // 0 = Sürekli ses
    ...
    RADAR_PRINTF("  [Buzzer] AKTİF. Aralık: %d ms\n", currentBeepInterval);
  } else {
    buzzerShouldBeActive = false;
    RADAR_PRINTLN("  [Buzzer] PASİF (Kapsama dışı)");
  }

  // 4. Ekranda Piksel Konumunu Hesaplama
  if (currentLateralRange_m < 0.1) currentLateralRange_m = 5.0; // Sıfıra bölünme hatasını önle
  
  // X konumu (Yanal): Oran orantı ile ekranın ortasından sapmayı bulur
  ...
  // Y konumu (İleri): Aracın burnuna olan yakınlık
...
void updateVehicleDisplay(float currentLateralRange_m) {
  // Ekranda çizilen "kendi aracımızın" genişliğini o anki zoom seviyesine göre ayarlar
  ...
  // Nextion'daki "rVehicle" bileşenini güncelle
  sendCommand("rVehicle.w=" + String(vehicle_width_px));
  delay(5); // Nextion bazen seri komutları kaçırabilir, çok kısa bekleme

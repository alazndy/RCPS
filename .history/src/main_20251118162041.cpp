 ...
 * ---------------------------------------------------------------------------------
 * Versiyon:    v3.4.0 (Kademeli Alarm Modu)
 * Yazar:       G.T (AI Tarafından Düzenlendi ve Yorumlandı)
 * Tarih:       18.11.2025
 *
 * AÇIKLAMA:
 * Bu kod, bir ESP32 kullanarak CAN bus üzerinden radar verilerini okur, işler ve
 * bir Nextion ekranda görselleştirir. Ayarlanabilir bölgelere, otomatik zooma ve
 * mesafeye göre ritmi değişen kademeli park sensörü alarmına sahiptir.
 *
 * --- v3.4.0 Değişiklik Notları ---
 * - ÖZELLİK (Büyük Güncelleme): Park sensörü mantığı, görsel alarm seviyelerine
 *              bağlı olarak kademeli çalışacak şekilde yeniden tasarlandı.
 *              - Sarı Alarm: Yavaş bip sesi ile başlar.
 *              - Turuncu Alarm: Bip hızı artar.
 *              - Kırmızı Alarm: Bip hızı çok artar.
 *              - 0.75m altında: Sürekli bir tona geçer.
 * - YAPI: Bip aralıkları her seviye için ayrı sabitler olarak tanımlandı.
 *         `handleDetection` içindeki alarm mantığı `if-else` yapısıyla güncellendi.
 * =================================================================================
 */
...
const int   BEEP_INTERVAL_YELLOW_MS = 400;  // Sarı Alarm (Uyarı) için bip aralığı.
const int   BEEP_INTERVAL_ORANGE_MS = 200;  // Turuncu Alarm (Tehlike) için bip aralığı.
const int   BEEP_INTERVAL_RED_MS    = 80;   // Kırmızı Alarm (Alarm) için bip aralığı.
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.4.0 - Kademeli Alarm Modu) --");
...
// (Bu bölümdeki sendCommand ve handleNextionInput fonksiyonları bir önceki versiyonla aynıdır.)
void sendCommand(String cmd) {
...
void handleDetection(const twai_message_t& msg) {
  ...
  if (autoZoom_enabled) {
      ...
      } else if (polarRadius_m > 1.5) { // Bu mesafe turuncu-kırmızı ayrımı için
      ...
      else { backgroundPicId = PIC_ID_ALARM; targetColor = COLOR_RED; } // Sabit zoomda Turuncu seviyesi yok
  }
  SERIAL_DEBUG_PRINTF("  [Görsel] Tehlike Seviyesi ID: %d, Aktif Yanal Menzil: %.1fm\n", backgroundPicId, currentLateralRange_m * 2.0);
  
  // --- 3. KADEMELİ BUZZER MANTIĞI ---
  // Alarm, sarı bölgeye (warningZone_m) girildiğinde başlar.
  if (audioAlarm_enabled && (polarRadius_m < warningZone_m) && (abs(doc_y_m) < (vehicleRealWidth_m / 2.0 + sideMargin_m))) {
    ...
    // En yüksek öncelik: Çok yakınsa sürekli ses ver.
    if (polarRadius_m <= SOLID_TONE_DISTANCE_M) {
      currentBeepInterval = 0; // Sürekli ses için özel değer
      SERIAL_DEBUG_PRINTF("  [Buzzer] KARAR: AKTİF (Sürekli Ses, Mesafe %.2fm <= %.2fm)\n", polarRadius_m, SOLID_TONE_DISTANCE_M);
    } 
    // Değilse, görsel alarm seviyesine göre bip hızını ayarla.
    else if (backgroundPicId == PIC_ID_ALARM) { // Kırmızı
      currentBeepInterval = BEEP_INTERVAL_RED_MS;
      SERIAL_DEBUG_PRINTF("  [Buzzer] KARAR: AKTİF (Kırmızı Alarm Bip Sesi, Aralık: %d ms)\n", currentBeepInterval);
    } 
    else if (backgroundPicId == PIC_ID_DANGER) { // Turuncu
      currentBeepInterval = BEEP_INTERVAL_ORANGE_MS;
      SERIAL_DEBUG_PRINTF("  [Buzzer] KARAR: AKTİF (Turuncu Alarm Bip Sesi, Aralık: %d ms)\n", currentBeepInterval);
    } 
    else if (backgroundPicId == PIC_ID_WARNING) { // Sarı
      currentBeepInterval = BEEP_INTERVAL_YELLOW_MS;
      SERIAL_DEBUG_PRINTF("  [Buzzer] KARAR: AKTİF (Sarı Alarm Bip Sesi, Aralık: %d ms)\n", currentBeepInterval);
    }
  } else {
    buzzerShouldBeActive = false;
    SERIAL_DEBUG_PRINTLN("  [Buzzer] KARAR: PASİF (Koşullar sağlanmadı veya Güvenli Bölge)");
  }
...
// =================================================================================
// BÖLÜM 7: GÜNCELLEME VE YARDIMCI FONKSİYONLAR
// =================================================================================
...
// =================================================================================
// BÖLÜM 8: EEPROM YÖNETİMİ (KALICI HAFIZA)
// =================================================================================

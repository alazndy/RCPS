 ...
 * ---------------------------------------------------------------------------------
 * Versiyon: v3.0.3 (Konumlandırma Düzeltmesi)
 * Yazar:    G.T
 * Tarih:    14.11.2025
 *
 * --- v3.0.3 Değişiklik Notları ---
 * - DÜZELTME (Kritik): handleDetection() fonksiyonundaki yatay pozisyon (targetX_px)
 *             hesaplaması, nesnenin hem sağa hem de sola doğru doğru şekilde
 *             konumlandırılmasını sağlamak için kontrol edildi ve doğrulandı.
 *             Sorun, yanal mesafe için doc_y_m değişkeninin doğru
 *             kullanılmamasıydı, bu hata giderildi.
 * =================================================================================
 */
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.0.3 - Düzeltilmiş) --");
...
void handleDetection(const twai_message_t& msg) {
  ...
  // YATAY HESAPLAMA - DOĞRU DEĞİŞKEN KULLANIMI KONTROL EDİLDİ

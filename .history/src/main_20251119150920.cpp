...
void handleNextionInput() {
  if (SerialNextion.available()) {
    ...
    rxBuffer[bytesRead] = '\0'; // String sonlandırıcı
    ...
    // --- SAVE3 (Sistem / Ses) - KRİTİK GÜNCELLEME ---
    else if (strncmp(rxBuffer, "SAVE3:", 6) == 0) {
      char* ptr = rxBuffer + 6;
      char* vZoom = strtok(ptr, ",");
      char* vAudio = strtok(NULL, ","); // Nextion'dan 1 veya 0 gelir
      ...
      if (vZoom && vAudio) {
        ...
        // EĞER: Ses açıktı ve şimdi kapatıldıysa -> ANINDA SUSTUR
        if (audioAlarm_enabled && !newAudioState) {
            digitalWrite(BUZZER_PIN, LOW); // Donanımsal olarak sus
            buzzerIsOn = false;            // Yazılımsal durumu güncelle
            buzzerShouldBeActive = false;  // Mantıksal durumu güncelle
            Serial.println("[INFO] Sesli Alarm kapatıldı, buzzer susturuldu.");
        }

 ...
 * ---------------------------------------------------------------------------------
 * Versiyon: v3.1.3 (Kararlılık ve EEPROM İyileştirmesi)
 * Yazar:    G.T (AI Tarafından Düzeltildi)
 * Tarih:    18.11.2025
 *
 * --- v3.1.3 Değişiklik Notları ---
 * - DÜZELTME (Kritik): `loadSettingsFromEEPROM` fonksiyonu, eski sürümden kalan
 *             geçersiz EEPROM verisi nedeniyle yanal menzil değişkenlerinin (lateralRange_L*)
 *             sıfır yüklenmesi sorununu gidermek için güncellendi. Artık EEPROM'daki
 *             ayar versiyonu uyumsuzsa, tüm ayarlar otomatik olarak varsayılana sıfırlanır.
 *             Bu, hedefin ekranda sağa yapışması sorununu çözer.
 * - GÜVENLİK: `handleDetection` fonksiyonuna, `currentLateralRange_m` değişkeninin
 *             beklenmedik bir şekilde sıfır olması durumunda sıfıra bölme hatasını
 *             önleyen bir koruma eklendi.
 * =================================================================================
 */
...
const int EEPROM_MAGIC_KEY = 124; // VERSİYON GÜNCELLENDİ (Eski ayarları geçersiz kılmak için)
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.1.3 - Kararlılık Düzeltmesi) --");
  ...
  if (twai_driver_install(&g_config, &t_config, &f_config) != ESP_OK || twai_start() != ESP_OK) {
    Serial.println("HATA: TWAI sürücüsü başlatılamadı. Donanımı kontrol edin.");
    while(1);
  }
...
void handleNextionInput() {
  if (SerialNextion.available()) {
    ...
    if (data.startsWith("LOGIN:")) {
      ...
      Serial.println("\n[DEBUG] Giriş denemesi: " + enteredPass);
...
void handleDetection(const twai_message_t& msg) {
  ...
  // `currentLateralRange_m` sıfır veya çok küçükse, sıfıra bölme hatasını engelle
  if (currentLateralRange_m < 0.1) {
    currentLateralRange_m = DEFAULT_MAX_WIDTH_M / 2.0; // Güvenli bir varsayılan değer kullan
  }
...
// ======================= EEPROM İŞLEMLERİ (DÜZELTİLDİ) =======================

void loadSettingsFromEEPROM() {
  EEPROM.begin(EEPROM_SIZE);
  // EEPROM'daki key, koddaki key ile uyuşmuyorsa, ayarlar geçersiz demektir.
  if (EEPROM.read(ADDR_MAGIC_KEY) != EEPROM_MAGIC_KEY) {
    ...
    resetToDefaults(); // Otomatik olarak sıfırla ve kaydet
  } else {
    Serial.println("[DEBUG] Geçerli ayarlar bulundu, EEPROM'dan okunuyor...");
  ...
  // Ayarları her zaman Nextion'a gönder (ya yeni sıfırlandı ya da okundu)
  sendSettingsToNextion();
}

void saveSettingsToEEPROM() {
  Serial.println("[DEBUG] Ayarlar EEPROM'a kaydediliyor...");
  EEPROM.put(ADDR_MAGIC_KEY, EEPROM_MAGIC_KEY); // Önce anahtarı yaz
...
void resetToDefaults() {
    Serial.println("[DEBUG] Varsayılan ayarlar yükleniyor ve kaydediliyor...");
    ...
    saveSettingsToEEPROM(); // Değişiklikleri hemen EEPROM'a kaydet
}

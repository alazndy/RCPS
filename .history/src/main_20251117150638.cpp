...
void updateTargetDisplay(int x, int y, int color) {
  if (!targetVisible) {
    sendCommand("vis rTarget,1"); // Değiştirildi
    targetVisible = true;
  }
  sendCommand("rTarget.pco=" + String(color)); // Değiştirildi
  sendCommand("rTarget.x=" + String(x));       // Değiştirildi
  sendCommand("rTarget.y=" + String(y));       // Değiştirildi
}

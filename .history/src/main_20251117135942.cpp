 ...
 * ---------------------------------------------------------------------------------
 * Versiyon: v3.0.1 (Tam ve Eksiksiz Sürüm)
 * Yazar:    G.T
 * Tarih:    14.11.2025
 *
 * --- v3.0.1 Değişiklik Notları ---
 * - DÜZELTME (Kritik): sendSettingsToNextion() fonksiyonunda eksik olan kapatma
 *             süslü parantezi '}' eklendi. Bu, derleme hatasını giderir.
 *
 * --- Kapsamlı Özellikler ---
 *  - Üç Sayfalı Ayarlar Menüsü ve Otomatik Kaydetme
 *  - EEPROM'a Kayıt: Tüm ayarlar (Mesafe, Genişlik, Şifre, Toggle'lar vb.) kalıcıdır.
 *  - İki Yönlü İletişim: Cihaz açılışında kayıtlı ayarlar Nextion'a gönderilir.
 *  - Güvenli Şifre Girişi ve Şifre Belirleme Ekranı Desteği.
 *  - Dinamik Grid ve Açılıp Kapatılabilir Otomatik Zoom.
 *  - Zoom'a Orantılı Dinamik Araç Genişliği.
 *  - Açılıp Kapatılabilir Sesli Alarm ve Yan Algılama Payı.
 *  - Tüm Ayarları Varsayılana Sıfırlama.
 *  - Detaylı Seri Monitör Hata Ayıklama Çıktıları.
 * =================================================================================
 */
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.0.1 - Düzeltilmiş) --");

...
// =================================================================================
// =================================================================================
// TAM VE EKSİKSİZ HABERLEŞME FONKSİYONU (Master Key + Debug + Buzzer Fix)
// =================================================================================
void handleNextionInput() {
  if (SerialNextion.available()) {
    // 1. VERİYİ OKUMA
    // Verinin parçalanmasını önlemek için biraz bekleme süresi
    SerialNextion.setTimeout(50); 
    int bytesRead = SerialNextion.readBytesUntil((char)0xFF, rxBuffer, RX_BUFFER_SIZE - 1);
    rxBuffer[bytesRead] = '\0'; // String sonlandırıcı ekle

    // Tamponda kalan diğer 0xFF'leri temizle
    ...
    // --- DEBUG: Nextion'dan ne geliyor? (Seri Monitörden Bakabilirsin) ---
    Serial.print("[DEBUG GELEN]: >");
    Serial.print(rxBuffer);
    Serial.println("<");

    char* cmdPtr; // Komutun başladığı yeri tutacak işaretçi

    // -----------------------------------------------------------------------------
    // 1. LOGIN KOMUTU (Master Key: 9999 Eklendi)
    // -----------------------------------------------------------------------------
    if ((cmdPtr = strstr(rxBuffer, "LOGIN:")) != NULL) {
      char* enteredPass = cmdPtr + 6; // "LOGIN:" sonrası
      
      // Görünmez karakterleri (boşluk, enter) temizle
      char* cleanPass = strtok(enteredPass, " \r\n");
      if(cleanPass == NULL) cleanPass = enteredPass;

      Serial.printf("[LOGIN DENEMESI] Girilen: '%s', Kayitli: '%s'\n", cleanPass, password);

      // KARŞILAŞTIRMA: Kayıtlı şifre VEYA "9999" (Master Key)
      if (strcmp(cleanPass, password) == 0 || strcmp(cleanPass, "9999") == 0) {
        Serial.println("[LOGIN] BASARILI. Giris yapiliyor...");
        sendSettingsToNextion();     
        sendCommand("page pageSet1"); 
      } else {
        Serial.println("[LOGIN] BASARISIZ.");
        sendCommand("tInfo.txt=\"Hatali Sifre\"");
        sendCommand("login_fail.val=1");          
      }
    }
    
    // -----------------------------------------------------------------------------
    // 2. SETPASS KOMUTU (Şifre Değiştirme)
    // -----------------------------------------------------------------------------
      ...
      if (cleanPass && strlen(cleanPass) > 0 && strlen(cleanPass) < 10) {
        ...
        Serial.printf("[SETPASS] Yeni sifre kaydedildi: %s\n", password);
      }
    }
    
    // -----------------------------------------------------------------------------
    // 3. SAVE1 (Bölge Ayarları)
    // -----------------------------------------------------------------------------
      ...
      if (val1 && val2) {
        ...
        saveSettingsToEEPROM();
        Serial.println("[SAVE1] Bolge ayarlari guncellendi.");
      }
    }
    
    // -----------------------------------------------------------------------------
    // 4. SAVE2 (Araç Ayarları)
    // -----------------------------------------------------------------------------
      ...
      if (vMargin && vWidth && vMax) {
        ...
        saveSettingsToEEPROM();
        Serial.println("[SAVE2] Arac ayarlari guncellendi.");
      }
    }
    
    // -----------------------------------------------------------------------------
    // 5. SAVE3 (Sistem Ayarları - SES KAPATMA FIX)
    // -----------------------------------------------------------------------------
      ...
      if (vZoom && vAudio) {
        ...
        Serial.printf("[SAVE3] Zoom: %d, Ses: %d\n", autoZoom_enabled, newAudioState);

        // KRİTİK: Ses kapatıldıysa Buzzer'ı donanımsal olarak hemen sustur
        if (!newAudioState) {
            digitalWrite(BUZZER_PIN, LOW);
            buzzerIsOn = false;
            buzzerShouldBeActive = false;
            Serial.println("[INFO] Buzzer zorla susturuldu.");
        }
    ...
    // -----------------------------------------------------------------------------
    // 6. RESETALL (Fabrika Ayarları)
    // -----------------------------------------------------------------------------
    else if (strstr(rxBuffer, "RESETALL") != NULL) {
      resetToDefaults();
      Serial.println("[RESET] Fabrika ayarlarina donuldu.");
    }
  }
}
// -------------------------------------------------------------------------------------------------

 ...
 * ---------------------------------------------------------------------------------
 * Versiyon: v3.0.4 (Radio Buton Entegrasyonu)
 * Yazar:    G.T
 * Tarih:    14.11.2025
 *
 * --- v3.0.4 Değişiklik Notları ---
 * - GÜNCELLEME: Hedef göstergesi, 'pTarget' (Button) yerine 'rTarget' (Radio)
 *             nesnesi ile değiştirildi.
 * - DÜZELTME: updateTargetDisplay() fonksiyonu, bir Radio butonunun rengini
 *             değiştirmek için '.bco' yerine doğru komut olan '.pco' kullanacak
 *             şekilde güncellendi.
 * - GÜNCELLEME: Kod içindeki tüm 'pTarget' referansları, yeni nesne adı olan
 *             'rTarget' ile değiştirildi.
 * =================================================================================
 */
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.0.4 - Radio Buton) --");
...
void handleDetection(const twai_message_t& msg) {
  ...
  // YENİ VE KRİTİK ADIM: Yeni hedefi çizmeden önce, bir önceki pozisyondaki
  // hedefi silmek için arka planı tamamen yeniden çiz. Bu, 'xstr' komutunun
  // arkasında iz bırakmasını engeller.
  sendCommand("page0.pic=" + String(backgroundPicId));

  // Diğer tüm elemanları (araç, metinler ve yeni hedef) bu temiz tuvalin üzerine çizdir
...
void clearDetection() {
  targetVisible = false;
  digitalWrite(BUZZER_PIN, LOW);
  sendCommand("vis rTarget,0"); // Değiştirildi
...
void updateTargetDisplay(int x, int y, int color) {
  // xstr komutu: x, y, genişlik, yükseklik, font, yazı rengi, arka plan rengi, x-merkez, y-merkez, durum, "metin"
  // Biz burada metin olarak özel bir karakter (dolu daire) kullanacağız.
  // Font 0'da genellikle bu karakterler bulunur. "●" veya "█" deneyebilirsiniz.
  // Genişlik/Yükseklik (w/h) ve Arka Plan Rengi (bco) 0 olacak çünkü sadece karakteri çiziyoruz.
  
  // Önce eski hedefi silmek için arka planı yeniden çiz
  // (Bu, hedefin "iz bırakmasını" engeller)
  // Bu komut handleDetection içine taşındığı için burada gerek kalmadı.
  
  // Şimdi yeni pozisyona hedefi çiz
  // Format: xstr x,y,w,h,font,pco,bco,xcen,ycen,sta,"text"
  String drawCommand = "xstr ";
  drawCommand += String(x) + ",";
  drawCommand += String(y) + ",";
  drawCommand += String(TARGET_OBJECT_SIZE_PX) + ","; // Genişlik
  drawCommand += String(TARGET_OBJECT_SIZE_PX) + ","; // Yükseklik
  drawCommand += "0,";  // Font ID
  drawCommand += String(color) + ","; // Yazı Rengi (Dairenin rengi)
  drawCommand += "0,";  // Arka Plan Rengi (Kullanılmıyor)
  drawCommand += "1,";  // X-Merkezli (Evet)
  drawCommand += "1,";  // Y-Merkezli (Evet)
  drawCommand += "0,";  // Arka plan yok (sadece karakter)
  drawCommand += "\"●\""; // Çizilecek karakter (Dolu Daire)
  
  sendCommand(drawCommand);
  
  // targetVisible bayrağını hala kullanıyoruz
  if (!targetVisible) {
    targetVisible = true;
  }
}
...
void clearDetection() {
  ...
  // DEĞİŞİKLİK: Hedefi silmek için artık 'vis' komutu yerine,
  // sadece arka plan resmini yeniden çiziyoruz. Bu, 'xstr' ile çizilen
  // her şeyi temizler.
  sendCommand("page0.pic=" + String(PIC_ID_SAFE)); 

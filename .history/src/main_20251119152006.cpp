...
void handleNextionInput() {
  if (SerialNextion.available()) {
    ...
    // --- BURASI GÜNCELLENDİ ---
      ...
      if (vZoom && vAudio) {
        int zoomVal = atoi(vZoom);
        int audioVal = atoi(vAudio);
        
        autoZoom_enabled = (zoomVal == 1);
        audioAlarm_enabled = (audioVal == 1);
        
        // Debug Mesajı: ESP32'nin ne anladığını görmek için
        Serial.printf("[DEBUG] Zoom: %d, Ses Durumu: %d (%s)\n", 
                      zoomVal, audioVal, audioAlarm_enabled ? "ACIK" : "KAPALI");
        
        // Eğer ses kapatıldıysa, anında donanımsal olarak sustur

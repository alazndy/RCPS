 ...
 * ---------------------------------------------------------------------------------
 * Versiyon:    v3.2.1 (Yorumlu Park Sensörü Modu Entegrasyonu)
 ...
 * gelen verileri okur. Okunan verileri işleyerek hedeflerin konumunu hesaplar ve
 * bu bilgileri bir Nextion HMI dokunmatik ekranda görselleştirir. Sistem, kullanıcı
 * tarafından ayarlanabilen tehlike bölgeleri, otomatik zoom ve mesafeye duyarlı
 * park sensörü tipi sesli alarm gibi gelişmiş özelliklere sahiptir.
 *
 * --- v3.2.1 Değişiklik Notları ---
 * - ÖZELLİK: Buzzer alarm sistemi, park sensörü gibi çalışacak şekilde yeniden
 *              tasarlandı. Cisim yaklaştıkça bip hızı artar ve 0.75m altında
 *              sürekli bir tona geçer.
 * - YAPI: Zamanlama `millis()` ile yönetilen, ana döngüyü engellemeyen yeni
 *         `handleBuzzer()` fonksiyonu eklendi.
 * - YORUMLAMA: Yeni eklenen tüm park sensörü mantığı detaylı olarak yorumlandı.
 * =================================================================================
 */
...
// =================================================================================
#include "driver/gpio.h"        // ESP32 pinlerini (GPIO) kontrol etmek için.
#include "driver/twai.h"        // ESP32'nin donanımsal CAN (TWAI) denetleyicisi için.
#include <math.h>               // Matematiksel hesaplamalar için (örn: abs).
#include <HardwareSerial.h>     // Nextion ekran ile seri haberleşme için.
#include <EEPROM.h>             // Ayarları kalıcı olarak saklamak için.
...
#define CAN_TX_PIN GPIO_NUM_5   // CAN Transceiver modülünün TX pini.
#define CAN_RX_PIN GPIO_NUM_4   // CAN Transceiver modülünün RX pini.
#define BUZZER_PIN 25           // Sesli alarm için buzzer pini.

// --- EEPROM Hafıza Haritası ve Ayarlar ---
...
// --- YENİ: Park Sensörü / Bip Mantığı için Sabitler ---
const float SOLID_TONE_DISTANCE_M = 0.75; // Sürekli sesin başlayacağı mesafe (metre).
const int   BEEP_ON_DURATION_MS   = 60;   // Her bir bip sesinin ne kadar süreceği (milisaniye).
const int   MAX_BEEP_INTERVAL_MS  = 500;  // En yavaş bip aralığı (uzaktayken).
const int   MIN_BEEP_INTERVAL_MS  = 50;   // En hızlı bip aralığı (yakındayken).
...
// --- YENİ: Park Sensörü / Bip Mantığı için Global Değişkenler ---
// Bu değişkenler, programın akışını durdurmadan (delay() kullanmadan)
// zamanlamalı bip sesi üretmek için kullanılır.
bool buzzerShouldBeActive = false; // Alarm koşullarının sağlanıp sağlanmadığını tutan bayrak.
bool buzzerIsOn           = false; // Buzzer pininin anlık durumunu (HIGH/LOW) tutar.
unsigned long lastBuzzerToggleTime = 0;   // Buzzer'ın durumunun en son ne zaman değiştiğini (ms) tutar.
int  currentBeepInterval  = MAX_BEEP_INTERVAL_MS; // Hesaplanan anlık bip aralığı (ms).
...
void handleBuzzer(); // Yeni park sensörü fonksiyonunun bildirimi
...
void setup() {
  ...
  Serial.println("\n-- Radar Alıcısı Başlatılıyor (v3.2.1 - Yorumlu Park Sensörü) --");
  ...
  Serial.println("TWAI (CAN) sürücüsü başarıyla başlatıldı.");
  clearDetection();
}

void loop() {
  // 1. Nextion'dan gelen komutları işle.
  handleNextionInput();

  // 2. CAN bus hattını dinle ve bir hedef algılanırsa işle.
  ...
  // 3. Hedef artık görünmüyorsa ekranı ve durumu temizle.
  if (!detectionThisCycle && targetVisible) {
    clearDetection();
  }

  // 4. YENİ: Her döngüde buzzer'ın bip sesini zamanlamaya göre yönet.
  // Bu fonksiyon, programın geri kalanından bağımsız olarak çalışır.
  handleBuzzer();
}
...
void handleDetection(const twai_message_t& msg) {
  // --- 1. Adım: Ham verileri anlamlı birimlere çevir. ---
  ...
  // --- 2. Adım: Tehlike seviyesini ve yanal menzili belirle. ---
  ...
  // --- 3. Adım: Sesli alarm durumunu ve bip hızını belirle (DEĞİŞTİRİLDİ). ---
  // Bu blok artık buzzer'ı doğrudan kontrol etmez, sadece handleBuzzer() fonksiyonu
  // için gerekli olan değişkenleri ayarlar.
  if (audioAlarm_enabled && (polarRadius_m < dangerZone_m) && (abs(doc_y_m) < (vehicleRealWidth_m / 2.0 + sideMargin_m))) {
    // Alarm koşulları sağlandı, buzzer'ın aktif olması gerektiğini belirt.
    buzzerShouldBeActive = true;
    
    // Mesafeye göre bip hızını hesapla:
    if (polarRadius_m <= SOLID_TONE_DISTANCE_M) {
      // Çok yakın, sürekli ses için özel bir değer (0) ata.
      currentBeepInterval = 0;
    } else {
      // Arduino'nun map() fonksiyonunu kullanarak mesafeyi, bip aralığına orantılı olarak dönüştür.
      // Cisim yaklaştıkça (polarRadius_m azalır), sonuç (currentBeepInterval) da azalır (bip hızlanır).
    ...
    // Alarm koşulları sağlanmıyor, buzzer'ın aktif olmaması gerektiğini belirt.
    buzzerShouldBeActive = false;
  }

  // --- 4. Adım: Metre koordinatlarını piksel koordinatlarına dönüştür. ---
  if (currentLateralRange_m < 0.1) {
    currentLateralRange_m = DEFAULT_MAX_WIDTH_M / 2.0;
  }
  ...
  // --- 5. Adım: Görsel arayüzü güncelle. ---
...
/**
 * @brief YENİ - Buzzer'ın bip sesini zamanlamaya göre yönetir.
 * Ana döngüde sürekli çağrılarak zamanı kontrol eder ve programı durdurmadan bip sesi üretir.
 */
void handleBuzzer() {
  // Eğer alarm durumu aktif değilse, buzzer'ı (eğer açıksa) kapat ve fonksiyondan çık.
  ...
  // Anlık zamanı milisaniye cinsinden al.
  unsigned long currentTime = millis();

  // --- Sürekli Ses Modu ---
  // Eğer bip aralığı özel değer olan 0 ise, buzzer'ı sürekli açık tut.
  if (currentBeepInterval == 0) {
    if (!buzzerIsOn) { // Sadece kapalıysa aç, gereksiz yere sürekli HIGH gönderme.
      digitalWrite(BUZZER_PIN, HIGH);
      buzzerIsOn = true;
    }
    return; // Sürekli açık kalacağı için başka bir işlem yapma.
  }

  // --- Bip Sesi Modu ---
  if (buzzerIsOn) {
    // Buzzer şu an AÇIK: Bip sesinin süresi (BEEP_ON_DURATION_MS) doldu mu diye kontrol et.
    if (currentTime - lastBuzzerToggleTime >= BEEP_ON_DURATION_MS) {
      digitalWrite(BUZZER_PIN, LOW); // Süre doldu, buzzer'ı kapat.
      buzzerIsOn = false;
      lastBuzzerToggleTime = currentTime; // Zamanlayıcıyı sıfırla.
    }
  } else {
    // Buzzer şu an KAPALI: Bipler arası bekleme süresi (currentBeepInterval) doldu mu diye kontrol et.
    if (currentTime - lastBuzzerToggleTime >= currentBeepInterval) {
      digitalWrite(BUZZER_PIN, HIGH); // Bekleme süresi doldu, yeni bir bip başlat.
      buzzerIsOn = true;
      lastBuzzerToggleTime = currentTime; // Zamanlayıcıyı sıfırla.
    }
  }
...
void clearDetection() {
  targetVisible = false;
  // Alarmı susturmak için bayrağı temizle. Gerçek susturma işlemini handleBuzzer() yapacak.
  buzzerShouldBeActive = false; 
...
void loadSettingsFromEEPROM() {
  ...
  if (EEPROM.read(ADDR_MAGIC_KEY) != EEPROM_MAGIC_KEY) {
    Serial.println("[!] Geçersiz EEPROM verisi bulundu. Tüm ayarlar varsayılana sıfırlanıyor...");
    resetToDefaults();
  } else {
    Serial.println("[OK] Geçerli ayarlar bulundu, EEPROM'dan okunuyor...");
...
void saveSettingsToEEPROM() {
  Serial.println("[SAVE] Ayarlar EEPROM'a kaydediliyor...");
  ...
  EEPROM.commit();
  Serial.println(" -> Kayıt başarıyla tamamlandı!");
}

void resetToDefaults() {
    Serial.println("[RESET] Varsayılan ayarlar yükleniyor ve kaydediliyor...");
...
void sendSettingsToNextion() {
  Serial.println("[SYNC] Mevcut ayarlar Nextion'a gönderiliyor...");
